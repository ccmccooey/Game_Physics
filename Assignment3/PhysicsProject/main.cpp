...
int main(int argc, char* argv[])
{
	...
	glutSpecialUpFunc(SpecialKeysUp); //special glut keyboard input when a key is up
	glutTimerFunc(16, FixedUpdate, 16); //16 is for 60 frames per second
	...
	glutKeyboardFunc(KeysDown); //keyboard input when a key is down
	glutKeyboardUpFunc(KeysUp); //keyboard input when a key is up
	glutIdleFunc(Update); //uncontrolled update, unfixed framerate
	atexit(Cleanup); //callback for when the application is closed
